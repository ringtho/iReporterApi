...
class TestRedFlags(unittest.TestCase):
    ...
    def test_create_redflag(self):
        response = self.test_client.post("/api/v1/red-flags", headers=dict(Authorization='Bearer '+ GetTokenTests.get_user_post(self)),
    ...
    #     .format(response.json["data"][0]["id"]) ,json=location)
    #     data = json.loads(res.data)
    #     print(data)
    ...
    #     res = self.test_client.patch("/api/v1/red-flags/3/comment" ,json=comment)
    #     data =json.loads(res.data)
    ...
    #     self.assertIn("The red flag record with id 1 doesnt exist", data["Error"])

    # def test_error_patch_invalid_id(self):
    #     response = self.test_client.post("/api/v1/red-flags", json=self.incident)
    #     self.assertEqual(response.status_code, 201)
    #     location = {"location": "mutungo"}
    #     response = self.test_client.patch("/api/v1/red-flags/4/location" ,json=location)
    #     data = json.loads(response.data)
    #     self.assertEqual(data["status"], 404)
